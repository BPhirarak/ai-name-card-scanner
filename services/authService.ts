...
export const validatePassword = (password: string): { valid: boolean; message: string } => {
  ...
  if (!hasLetter || !hasNumberOrSpecial) {
    return { valid: false, message: 'รหัสผ่านต้องประกอบด้วยตัวอักษรและตัวเลขหรืออักขระพิเศษ' };
  }
...
// Sign in user
export const signIn = async (username: string, password: string): Promise<{ success: boolean; message: string; user?: User }> => {
  try {
