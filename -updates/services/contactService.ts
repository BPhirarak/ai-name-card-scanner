...
export const saveCard = async (
  ...
  try {
    console.log('💾 Saving card...', { card, hasImageData: !!card.originalImageData });
    ...
    if (card.originalImageData && card.originalImageMimeType) {
      console.log('📸 Processing image upload...');
      
      try {
        // Convert base64 to File
        const fileName = generateFileName(cardId, card.originalImageMimeType);
        const imageFile = base64ToFile(card.originalImageData, card.originalImageMimeType, fileName);
        
        // Upload image to Firebase Storage
        const imageUrl = await uploadImageToStorage(imageFile, fileName, onProgress);
    ...
    delete cardToSave.originalImageData;
    delete cardToSave.originalImageMimeType;
...
export const deleteCard = async (cardId: string): Promise<void> => {
  try {
    ...
    if (cardData?.imageUrl && cardData?.imageFileName) {
      try {
        console.log('🗑️ Deleting associated image...');
        const imageRef = storageRef(storage, `business-cards/${cardData.imageFileName}`);
